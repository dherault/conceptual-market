...
Concepts would be created as soon as a news article mentions it. Once enough data is collected to compute a first initial index for a concept, a number of shares for that concept would be emitted, and the concept would become tradable. The initial shares would be bought from the platform. Then, the shares would be traded with buy and sell orders between traders, similar to a bid-ask system. The platform would take a trading fee on these transactions, typically 2%.

## The payoff

When a transaction is accepted, part of the platform’s fee is accumulated in a pool, each concept having its own pool. If, at the end of the period (possibly a week), the index has grown compared to the previous period, then the pool is distributed among the shareholders. So a shareholder is not just betting on a price going up, but also on the velocity of the conversation.
